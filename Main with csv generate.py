...
def PArseTXTFromDir(dir):
    ...
    for file in files:
        ...
        with open(dir + "/" + file, 'r', encoding="utf8") as fh:
            lines = fh.readlines()
...
def makeWordsTxt(freq):
    ...
    temp.sort()
    with open('words.txt','w',encoding="utf8") as out:
        for i in temp:
...
if __name__ == '__main__':
    ...
    for word in freq:
        ...
        year += [int(yearOfFile[id])]
        count += [freqPerFile[id][word]]
    ...
    nx.draw(G,pos = nx.nx_pydot.graphviz_layout(G,'fdp',),with_labels = True,width = weights, node_size = nodeSizes, node_color = nodeColors, edge_color = edgeColors,encoding="utf8")
